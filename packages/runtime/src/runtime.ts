...
export class AgentRuntime {
  ...
  async runTurn(skill: SkillBundle, message: string): Promise<AgentRun> {
    ...
    let lastRequest: ToolRequest | undefined;
    let lastPolicy: AgentRun["policyDecision"];

    for (let iteration = 0; iteration <= this.maxToolSteps; iteration += 1) {
      const decision = await this.model.decide(message, { skill, history });
      usage = addUsage(usage, decision.usage);
      modelName = decision.model ?? modelName;
      steps.push({
        id: makeId("step"),
        kind: "model",
        title: iteration === 0 ? "Specialist planned next action" : "Specialist evaluated tool result",
        status: "completed",
      ...
      if (!decision.tool) {
        return {
          id: runId, skill: skill.manifest.name, message, status: "completed",
          response: decision.response, steps, toolRequest: lastRequest, policyDecision: lastPolicy,
          model: modelName, usage,
        };
      }

      if (iteration === this.maxToolSteps) {
        return {
          id: runId, skill: skill.manifest.name, message, status: "failed",
          response: "Tool-step limit reached before the task completed.", steps,
          toolRequest: lastRequest, policyDecision: lastPolicy, model: modelName, usage,
        };
      }
      ...
      if (policy === "deny") {
        ...
        return {
          ...
          steps, toolRequest: request, policyDecision: policy, model: modelName, usage,
        };
      }
      ...
      if (policy === "approval_required") {
        ...
        return {
          id: runId, skill: skill.manifest.name, message, status: "waiting_approval",
          response: decision.response, steps, toolRequest: request, policyDecision: policy,
          model: modelName, usage,
        };
      }
      ...
      if (!result.ok) {
        return {
          id: runId, skill: skill.manifest.name, message, status: "failed",
          response: `Tool ${request.tool} failed.`, steps, toolRequest: request,
          policyDecision: policy, model: modelName, usage,
        };
      }
